        ...
        For Y = 1 To 100
            ...
            If InMapBounds(UserPos.X + X, UserPos.Y + Y) Then
                ...
                With MapData_Copy(UserPos.X + X, UserPos.Y + Y)
                    ...
                    If .Graphic(1).GrhIndex <> 0 Then Draw_Grh .Graphic(1), (X - 1) * 32, (Y - 1) * 32, To_Depth(1), 1, 1, , -1, , eTechnique.t_Alpha, GrhData(.Graphic(1).GrhIndex).pixelWidth, GrhData(.Graphic(1).GrhIndex).pixelHeight
                            
                    If .Graphic(2).GrhIndex <> 0 Then Draw_Grh .Graphic(2), (X - 1) * 32, (Y - 1) * 32, To_Depth(2, X, Y), 1, 1, , -1, , eTechnique.t_Alpha, GrhData(.Graphic(2).GrhIndex).pixelWidth, GrhData(.Graphic(2).GrhIndex).pixelHeight
                                
                    If .Graphic(3).GrhIndex <> 0 Then Draw_Grh .Graphic(3), (X - 1) * 32, (Y - 1) * 32, To_Depth(3, X, Y), 1, 1, , -1, , eTechnique.t_Alpha, GrhData(.Graphic(3).GrhIndex).pixelWidth, GrhData(.Graphic(3).GrhIndex).pixelHeight
                               
                    If .Graphic(4).GrhIndex <> 0 Then Draw_Grh .Graphic(4), (X - 1) * 32, (Y - 1) * 32, To_Depth(4, X, Y), 1, 1, , -1, , eTechnique.t_Alpha, GrhData(.Graphic(4).GrhIndex).pixelWidth, GrhData(.Graphic(4).GrhIndex).pixelHeight
    ...
    For X = 1 To 100
        For Y = 1 To 100
            ...
            With MapData_Copy(X, Y)
                ...
                If .Graphic(1).GrhIndex <> 0 Then Draw_Grh .Graphic(1), (X - 1) * 32, (Y - 1) * 32, To_Depth(1), 1, 1, , -1, , eTechnique.t_Alpha
...
Public Sub Render_CharAccount()
    ...
    Call wGL_Graphic.Use_Device(g_Captions(eCaption.eCharAccount))
    ...
    'Draw_Text f_Medieval, 75, 400, 25, To_Depth(5), 0, ARGB(250, 157, 30, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Desterium AO", True
    'Draw_Text f_Medieval, 75, 402, 27, To_Depth(5), 0, ARGB(255, 255, 255, 50), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Desterium AO", False
    
    AlphaTexture = ARGB(255, 255, 255, 255)
    
    Select Case Account_PanelSelected
        ...
        Case eAccount_PanelSelected.ePrincipal, eAccount_PanelSelected.ePanelAccountRecover, eAccount_PanelSelected.ePanelAccountRegister
            Draw_Text f_Medieval, 30, 400, 65, To_Depth(7), 0, ARGB(250, 255, 255, 175), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "V4.0.0", False
            InitialX = 30
            InitialY = 25
            AlphaTexture = ARGB(255, 255, 255, 255)
            
            Call Render_Principal
        ...
        Case eAccount_PanelSelected.ePanelAccount
            ...
            'Draw_Text f_Medieval, 25, 950, 180, To_Depth(6), 0, ARGB(250, 255, 255, 175), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Panel de la Cuenta", False
            
            InitialX = 59
            InitialY = 60
            Call Render_Account
            AlphaTexture = ARGB(255, 255, 255, 200)
            
            Pixel = 32
            Divition = 1

            ' Creación de un Nuevo Personaje
        Case eAccount_PanelSelected.ePanelAccountCharNew
...
#If ModoBig > 0 Then
    ...
    Private Sub Render_Account()
        ...
        Draw_Text f_Tahoma, 19, 140, 95, To_Depth(3), 0, ARGB(250, 250, 100, 175), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_LEFT, IIf(Account.Gld > 0, Format$(Account.Gld, "##,##") & "  ORO", "0"), False, True
        Draw_Text f_Tahoma, 19, 140, 140, To_Depth(3), 0, ARGB(250, 250, 100, 175), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_LEFT, IIf(Account.Eldhir > 0, Format$(Account.Eldhir, "##,##") & " DSP", "0"), False, True
        Draw_Text f_Tahoma, 19, 140, 185, To_Depth(3), 0, ARGB(250, 250, 100, 175), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_LEFT, IIf(UserPoints > 0, Format$(UserPoints, "##,##") & " P. Torneo", "0"), False, True
        ...
        For A = 1 To 10
            ...
            With Account.Chars(A)

                If .PosMap > 0 Then
                    ' Recuadro del Personaje
                    Call Draw_Texture_Graphic_Gui(25, x1 + X - 80, y1 + Y - 70, To_Depth(4), 339 / Mult, 272 / Mult, 0, 0, 339 / Mult, 272 / Mult, -1, 0, eTechnique.t_Alpha)

                    If .Blocked > 0 Then
                        Call Draw_Texture_Graphic_Gui(27, X - 25, Y - 15, To_Depth(5), 16, 16, 0, 0, 16, 16, -1, 0, eTechnique.t_Alpha)

                    End If
                
                    ' Mapa del Personaje
                    ' ADAPTAR CLASICO
                    Call Draw_Texture_Graphic_Gui(62, X + 50 + 80, Y + 75, To_Depth(5), 32, 32, 0, 0, 32, 32, -1, 0, eTechnique.t_Alpha)
                    Draw_Text f_Tahoma, 14, X + 66 + 80, Y + 10 + 75, To_Depth(7), 0, -1, FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, .PosMap, True, True
                
                    ' Recuadro del Personaje seleccionado
                    If Account.SelectedChar = Account.Chars(A).ID Then
                        Call Draw_Texture_Graphic_Gui(113, x1 + X - 80, y1 + Y - 70, To_Depth(5), 339 / Mult, 272 / Mult, 0, 0, 339 / Mult, 272 / Mult, -1, 0, eTechnique.t_Alpha)

                    End If
                
                    ColourText = ARGB(255, 255, 255, 255)
                    ...
                    ' Cuadrado Style Mu
                    Draw_Text f_Booter, 30, X + 117, y1 + Y - 26, To_Depth(7), 0, ColourText, FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, ListaClases(.Class) & " " & ListaRazas(.Raze), True, True
                    Draw_Text f_Booter, 30, X + 117, y1 + Y - 6, To_Depth(7), 0, ColourText, FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Nivel " & .Elv, True, True
                    Draw_Text f_Booter, 25, X + 110, Y + 150, To_Depth(7), 0, .Colour, FONT_ALIGNMENT_CENTER, .Name, True, True
                
                    Dim AlphaBody As Long
                
                    If .Body <> FRAGATA_FANTASMAL And .Body <> iCuerpoMuerto And .Body <> iCuerpoMuerto_Legion Then
                        AlphaBody = ARGB(255, 255, 255, 255)
                    
                        If .Head > 0 Then Call Draw_Grh(HeadData(.Head).Head(E_Heading.SOUTH), X + BodyData(.Body).HeadOffset.X + 80, Y + BodyData(.Body).HeadOffset.Y + 75, To_Depth(7), 1, 0, , , , , GrhData(HeadData(.Head).Head(E_Heading.SOUTH).GrhIndex).pixelWidth, GrhData(HeadData(.Head).Head(E_Heading.SOUTH).GrhIndex).pixelHeight)
                
                        If .Helm > 0 And .Helm <> 2 Then
                            If .Helm = 54 Then
                                OffsetY = 13
                            Else
                                OffsetY = 0
                            ...
                            Call Draw_Grh(CascoAnimData(.Helm).Head(E_Heading.SOUTH), X + BodyData(.Body).HeadOffset.X + 80, Y + BodyData(.Body).HeadOffset.Y + OffsetY + 75, To_Depth(7, , , 3), 1, 0, , , , , GrhData(CascoAnimData(.Helm).Head(E_Heading.SOUTH).GrhIndex).pixelWidth, GrhData(CascoAnimData(.Helm).Head(E_Heading.SOUTH).GrhIndex).pixelHeight)

                        End If
                
                        If .Shield > 0 And .Shield <> 2 Then Call Draw_Grh(ShieldAnimData(.Shield).ShieldWalk(E_Heading.SOUTH), X + 80, Y + 75, To_Depth(7, , , 4), 1, 1, 0, , , , (GrhData(ShieldAnimData(.Shield).ShieldWalk(E_Heading.SOUTH).GrhIndex).pixelWidth / 2), (GrhData(ShieldAnimData(.Shield).ShieldWalk(E_Heading.SOUTH).GrhIndex).pixelHeight / 2))
                            
                        If .Weapon > 0 And .Weapon <> 2 Then Call Draw_Grh(WeaponAnimData(.Weapon).WeaponWalk(E_Heading.SOUTH), X + 80, Y + 75, To_Depth(8), 1, 1, 0, , , , GrhData(WeaponAnimData(.Weapon).WeaponWalk(E_Heading.SOUTH).GrhIndex).pixelWidth / 2, GrhData(WeaponAnimData(.Weapon).WeaponWalk(E_Heading.SOUTH).GrhIndex).pixelHeight / 2)
                    Else
                        AlphaBody = ARGB(255, 255, 255, 150)

                    End If
                
                    Call Draw_Grh(BodyData(.Body).Walk(E_Heading.SOUTH), X + 80, Y + 75, To_Depth(6), 1, 0, 0, AlphaBody, , eTechnique.t_Alpha, GrhData(BodyData(.Body).Walk(E_Heading.SOUTH).GrhIndex).pixelWidth / 2, GrhData(BodyData(.Body).Walk(E_Heading.SOUTH).GrhIndex).pixelHeight / 2)
            ...
            If QuestNpcIndex > 0 Then
                ...
                If .Npc > 0 Then
                    ...
                    If NpcList(.Npcs(QuestNpcIndex).NpcIndex).Head > 0 Then
                        Call Draw_Grh(HeadData(NpcList(.Npcs(QuestNpcIndex).NpcIndex).Head).Head(E_Heading.EAST), 85 + BodyData(NpcList(.Npcs(QuestNpcIndex).NpcIndex).Body).HeadOffset.X, 420 + BodyData(NpcList(.Npcs(QuestNpcIndex).NpcIndex).Body).HeadOffset.Y, To_Depth(5), 1, 0, 0)
...
Public Sub Render_CriaturaInfo()
    ...
    With NpcList(SelectedNpcIndex)
        ...
        If .Head > 0 Then
            Call Draw_Grh(HeadData(.Head).Head(E_Heading.EAST), 85 + BodyData(.Body).HeadOffset.X, 200 + BodyData(.Body).HeadOffset.Y, To_Depth(5), 1, 0, 0)
        ...
        ' Vida de la Criatura
        If .MaxHit > 0 Then
            Call Draw_Text(eFonts.f_Morpheus, 20, 75, Y_Avance, To_Depth(9), 0, ARGB(200, 200, 175, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Hp", True)
        
            Call Draw_Text(eFonts.f_Morpheus, 20, 140, Y_Avance, To_Depth(9), 0, ARGB(255, 200, 0, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, Format$(.MaxHp, "#,###"), True)
        ...
        If .MinHit > 0 Then
            ...
            Call Draw_Text(eFonts.f_Morpheus, 20, 75, Y_Avance, To_Depth(9), 0, ARGB(200, 200, 175, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Hit", True)
        
            Call Draw_Text(eFonts.f_Morpheus, 20, 140, Y_Avance, To_Depth(9), 0, ARGB(255, 200, 0, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, Format$(.MinHit, "#,###") & "/" & Format$(.MaxHit, "#,###"), True)
        ...
        If .Def > 0 Then
            Call Draw_Text(eFonts.f_Morpheus, 20, 75, Y_Avance, To_Depth(9), 0, ARGB(200, 200, 175, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Def", True)
        
            Call Draw_Text(eFonts.f_Morpheus, 20, 140, Y_Avance, To_Depth(9), 0, ARGB(255, 200, 0, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, Format$(.Def, "#,###"), True)
            Y_Avance = Y_Avance + 20
        ...
        If .DefM > 0 Then
            Call Draw_Text(eFonts.f_Morpheus, 20, 75, Y_Avance, To_Depth(9), 0, ARGB(200, 200, 175, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Def Mag", True)
        
            Call Draw_Text(eFonts.f_Morpheus, 20, 140, Y_Avance, To_Depth(9), 0, ARGB(255, 200, 0, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, Format$(.DefM, "#,###"), True)
        ...
        If .PoderEvasion > 0 Then
            Call Draw_Text(eFonts.f_Morpheus, 20, 75, Y_Avance, To_Depth(9), 0, ARGB(200, 200, 175, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Evasion", True)
        
            Call Draw_Text(eFonts.f_Morpheus, 20, 140, Y_Avance, To_Depth(9), 0, ARGB(255, 200, 0, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, Format$(.PoderEvasion, "#,###"), True)
        ...
        If .PoderAtaque > 0 Then
            ...
            Call Draw_Text(eFonts.f_Morpheus, 20, 140, Y_Avance, To_Depth(9), 0, ARGB(255, 200, 0, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, Format$(.PoderAtaque, "#,###"), True)
        ...
        ' Experiencia
        If .GiveExp > 0 Then
            Call Draw_Text(eFonts.f_Morpheus, 20, 75, Y_Avance, To_Depth(9), 0, ARGB(200, 200, 175, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Exp", True)
        
            Call Draw_Text(eFonts.f_Morpheus, 20, 140, Y_Avance, To_Depth(9), 0, ARGB(50, 200, 200, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, Format$(.GiveExp, "#,###"), True)
        ...
        ' Oro
        If .GiveGld > 0 Then
            Call Draw_Text(eFonts.f_Morpheus, 20, 75, Y_Avance, To_Depth(9), 0, ARGB(200, 200, 175, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Oro", True)
        
            Call Draw_Text(eFonts.f_Morpheus, 20, 140, Y_Avance, To_Depth(9), 0, ARGB(50, 200, 200, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, Format$(.GiveGld, "#,###"), True)
        ...
        If .NroItems > 0 Then
            ...
            Call Draw_Text(eFonts.f_Morpheus, 20, 105, 220, To_Depth(9), 0, ARGB(255, 200, 0, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Inv", True)
            ...
            For A = 1 To 6
                ...
                If .Object(A).ObjIndex > 0 Then
                    Call Draw_Text(eFonts.f_Tahoma, 10, X_ITEM, Y_ITEM, To_Depth(9), 0, ARGB(255, 255, 255, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_LEFT, "x" & .Object(A).Amount, True)
        ...
        If .NroDrops > 0 Then
            ...
            Call Draw_Text(eFonts.f_Morpheus, 20, 105, 272, To_Depth(9), 0, ARGB(40, 150, 40, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_CENTER, "Drop", True)
            ...
            For A = 1 To 6
                ...
                If .Drop(A).ObjIndex > 0 Then
                    Call Draw_Text(eFonts.f_Tahoma, 10, X_ITEM, Y_ITEM, To_Depth(9), 0, ARGB(255, 255, 255, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_LEFT, Porcentaje_String(.Drop(A).Probability), True)
                    Call Draw_Text(eFonts.f_Tahoma, 10, X_ITEM, Y_ITEM + 22, To_Depth(9), 0, ARGB(255, 255, 255, 255), FONT_ALIGNMENT_TOP Or FONT_ALIGNMENT_LEFT, "x" & .Drop(A).Amount, True)
...
RenderScreen_Err:
    LogError err.Description & vbCrLf & "in RenderScreen_MiniMapa " & "at line " & Erl
...
Public Function ComprobarPosibleMacro(ByVal MouseX As Integer, _
                                      ByVal MouseY As Integer) As Boolean
...
Public Sub CountPacketIterations(ByRef packetControl As t_packetControl, _
                                 ByVal expectedAverage As Double)
    ...
    If percentageDiff < 5 Then
        ...
        Call WriteDenounce("[SEGURIDAD]: Está repitiendo un Macro" & getPercentageDiff(packetControl))

        'Debug.Print "DIFF: " & getPercentageDiff(packetControl)
    End If
    
    If average > 50 And average < expectedAverage Then
        Call WriteDenounce("[SEGURIDAD]: Está repitiendo un Macro" & getPercentageDiff(packetControl))
...
CountPacketIterations_Err:
    LogError err.Description & vbCrLf & "in ARGENTUM.Engine_Renders.LaEscaloneta " & "at line " & Erl
