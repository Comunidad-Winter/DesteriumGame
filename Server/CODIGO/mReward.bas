...
Public Sub Reward_Process_User(ByVal ModalityID As Byte, ByRef Player As tPlayerData)
...
Private Function Reward_InventorySlot_Offline(ByRef Manager As clsIniManager, _
...
Public Function Bonus_User_SearchSlot_Offline(ByVal BonusLast As Integer, _
                                              ByRef Manager As clsIniManager) As Integer
...
Public Sub Bonus_AddUser_Offline(ByVal Name As String, _
    ...
    With Stats
        ...
        Call Manager.ChangeValue("BONUS", "BONUS" & Slot, CStr(Tipo) & "|" & CStr(Value) & "|" & CStr(Amount) & "|" & CStr(DurationSeconds) & "|" & CStr(DurationDate))
