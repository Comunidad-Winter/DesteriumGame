...
Private Function Fight_ChangeMap(ByVal Slot As Byte, ByVal Terreno As Byte) As Integer
...
Private Sub ResetDueloUser(ByVal UserIndex As Integer, _
    ...
    With UserList(UserIndex)
        ...
        With Retos(.flags.SlotReto)
            .User(UserList(UserIndex).flags.SlotRetoUser).UserIndex = 0
            .User(UserList(UserIndex).flags.SlotRetoUser).Team = 0
            .User(UserList(UserIndex).flags.SlotRetoUser).Accepts = 0
            .User(UserList(UserIndex).flags.SlotRetoUser).Rounds = 0
...
error:
    LogRetos "[" & Err.number & "] " & Err.description & ") PROCEDIMIENTO : ResetDueloUser() userindex: " & UserIndex
    '<EhFooter>
    Exit Sub
...
Fight_FreeSlot_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mRetos.Fight_FreeSlot " & "at line " & Erl
...
Private Function ValidateFight_Hack(ByVal UserIndex As Integer, _
    ...
    With Fight
        ...
        If .Gld < 50000 Or .Gld > 100000000 Then
            LogRetos "POSIBLE HACKEO: " & UserList(UserIndex).Name & " hackeo el sistema de retos::GLD"
    
            Exit Function
    
        End If
    
        If .Time <> 300 And .Time <> 600 And .Time <> 1200 And .Time <> 1800 And .Time <> 3600 Then
            LogRetos "POSIBLE HACKEO: " & UserList(UserIndex).Name & " hackeo el sistema de retos::TIME"
    
            Exit Function
    
        End If
    
        If .RoundsLimit <> 1 And .RoundsLimit <> 3 And .RoundsLimit <> 5 And .RoundsLimit <> 10 And .RoundsLimit <> 20 Then
            LogRetos "POSIBLE HACKEO: " & UserList(UserIndex).Name & " hackeo el sistema de retos::ROUNDS"
        ...
        LongitudUsers = UBound(.User) + 1
        
        If LongitudUsers <> 2 And LongitudUsers <> 4 And LongitudUsers <> 6 And LongitudUsers <> 8 And LongitudUsers <> 10 Then
            Call WriteConsoleMsg(UserIndex, "Solicitud Inválida. Has puesto mal los nombres de los personajes.", FontTypeNames.FONTTYPE_INFORED)
            Exit Function
        ...
        If UBound(.User) > 1 Then
            If .Tipo = 4 Then
                LogRetos "POSIBLE HACKEO: " & UserList(UserIndex).Name & " hackeo el sistema de retos::PLANTES"
    
                Exit Function
    
            End If
            
            If .config(eRetoConfig.eItems) > 0 Then
                LogRetos "POSIBLE HACKEO: " & UserList(UserIndex).Name & " hackeo el sistema de retos::ITEMS"
        ...
        If UBound(.User) <= 1 Then
            If .config(eRetoConfig.eResucitar) > 0 Then
                LogRetos "POSIBLE HACKEO: " & UserList(UserIndex).Name & " hackeo el sistema de retos::RESUCITAR"
        ...
        If UserList(UserIndex).Counters.FightSend > 0 Then
            Call WriteConsoleMsg(UserIndex, "Has enviado una solicitud recientemente. Aguarda unos instantes...", FontTypeNames.FONTTYPE_INFORED)
        ...
        ReDim User(LBound(.User) To UBound(.User)) As tFightUser
        
        For A = LBound(.User) To UBound(.User)
            tUser = NameIndex(.User(A).Name)
            
            If StrComp(UCase$(.User(A).Name), UCase$(UserList(UserIndex).Name)) = 0 Then
                FoundSender = True

            End If
            
            If EsDios(.User(A).Name) Or EsAdmin(.User(A).Name) Or EsSemiDios(.User(A).Name) Then
            ...
            If tUser <= 0 Then
                Call WriteConsoleMsg(UserIndex, "Uno de los personajes no está disponible.", FontTypeNames.FONTTYPE_INFORED)
            ...
            If tUser > 0 Then
                User(A).UserIndex = tUser
                User(A).Name = UserList(tUser).Name
                ...
                If UserList(tUser).Counters.FightInvitation > 0 Then
                    Call WriteConsoleMsg(UserIndex, "El personaje " & .User(A).Name & " está por aceptar otra invitación", FontTypeNames.FONTTYPE_INFORED)
        ...
        If Not FoundSender Then
            ValidateFight_Hack = False
            Exit Function
        Else
            .User = User 'Cargamos la temporal validada

        End If
        
    End With
    
    ValidateFight_Hack = True
    '<EhFooter>
    Exit Function

ValidateFight_Hack_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mRetos.ValidateFight_Hack " & "at line " & Erl
...
Private Sub SendInvitation(ByVal UserIndex As Integer, ByRef Fight As tFight)
...
Private Function Fight_TextUsers(ByRef User() As tFightUser) As String
    ...
    For A = LBound(User) To UBound(User)
        tUser = User(A).UserIndex
        
        If tUser > 0 Then
            Text = Text & User(A).Name & " (" & ListaClases(UserList(tUser).Clase) & " " & ListaRazas(UserList(tUser).Raza) & ") Lvl " & UserList(tUser).Stats.Elv & vbCrLf & SEPARATOR

        End If

    Next A
    
    If LenB(Text) > 0 Then Text = Left$(Text, Len(Text) - 1)
    
    Fight_TextUsers = Text
    '<EhFooter>
    Exit Function

Fight_TextUsers_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mRetos.Fight_TextUsers " & "at line " & Erl
...
Private Sub WarpFight(ByVal Slot As Byte, ByVal OrdenTeam As Byte)
    ...
    With Retos(Slot)
        ...
        For LoopC = LBound(.User()) To UBound(.User())
            tUser = .User(LoopC).UserIndex
                  
            If tUser > 0 Then
                If Not OrdenTeam = 0 Then ' No warpeamos pero stopeamos
                    Slot = UserList(tUser).flags.SlotReto
                    SlotArena = Retos(Slot).Arena
                    
                    Pos.Map = Arenas(SlotArena).Map
                    
                    If .User(LoopC).Team = OrdenTeam Then
                        Pos.X = Arenas(SlotArena).X
                        Pos.Y = Arenas(SlotArena).Y
                    Else
                        Pos.X = Arenas(SlotArena).X + Arenas(SlotArena).TileAddX
                        Pos.Y = Arenas(SlotArena).Y + Arenas(SlotArena).TileAddY

                    End If
                          
                    With UserList(tUser)
                        ClosestStablePos Pos, Pos
                        WarpUserChar tUser, Pos.Map, Pos.X, Pos.Y, False

                    End With
        
                End If
                
                UserList(tUser).Counters.TimeFight = 10
...
WarpFight_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mRetos.WarpFight " & "at line " & Erl
    ...
    With Retos(SlotReto)
        ...
        For LoopC = LBound(.User()) To UBound(.User())
            ...
            If .User(LoopC).UserIndex > 0 Then
                ...
                If .User(LoopC).UserIndex > 0 Then
                    If UBound(.User) = 1 Then
                        Call WriteUpdateEvent(UserList(.User(LoopC).UserIndex).ID, UserList(.User(LoopC).UserIndex).Name, eSubType_Modality.Retos1vs1, Team = .User(LoopC).Team)
        ...
        Else
            ...
            SendData SendTarget.toMap, 1, PrepareMessageConsoleMsg(IIf((.Tipo = 4), "«Plantes» ", "«Retos» ") & IIf((.RoundsLimit = 1), "¡A 1 Round! ", "¡A " & .RoundsLimit & " Rounds! ") & StrTeam(SlotReto) & ". Ganador " & strTemp & ". Apuesta por " & .Gld & " Monedas de Oro." & IIf((.config(eRetoConfig.eItems) > 0), " ¡¡Le usurpó el inventario!!", vbNullString), FontTypeNames.FONTTYPE_INFORETOS)
...
Public Sub SendFight(ByVal UserIndex As Integer, ByRef Fight As tFight)
...
Private Function Retos_Obj_NotWin(ByVal ObjIndex As Integer) As Boolean
    ...
    With ObjData(ObjIndex)
        ...
        If ObjData(ObjIndex).OBJType <> otBarcos And ObjData(ObjIndex).OBJType <> otPociones And ObjData(ObjIndex).OBJType <> otBebidas And ObjData(ObjIndex).OBJType <> otUseOnce And ObjData(ObjIndex).OBJType <> otBotellaVacia And ObjData(ObjIndex).OBJType <> otLeña And ObjData(ObjIndex).OBJType <> otMinerales And ObjData(ObjIndex).OBJType <> otBotellaLlena And ObjData(ObjIndex).Real <> 1 And ObjData(ObjIndex).Caos <> 1 And ObjData(ObjIndex).Premium <> 1 And ObjData(ObjIndex).Plata <> 1 And ObjData(ObjIndex).Oro <> 1 And ObjData(ObjIndex).NoNada <> 1 Then
                    
            Retos_Obj_NotWin = True
...
Retos_Obj_NotWin_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mRetos.Retos_Obj_NotWin " & "at line " & Erl
...
Sub Retos_SetObj(ByVal UserIndex As Integer, ByVal VictimIndex As Integer)
    ...
    With UserList(VictimIndex)
        ...
        For A = 1 To .CurrentInventorySlots
            ObjIndex = .Invent.Object(A).ObjIndex
            
            If ObjIndex > 0 Then
                If Retos_Obj_NotWin(ObjIndex) Then
                    
                    UserList(UserIndex).ObjectClaim(A).ObjIndex = .Invent.Object(A).ObjIndex
                    UserList(UserIndex).ObjectClaim(A).Equipped = 0
                    UserList(UserIndex).ObjectClaim(A).Amount = .Invent.Object(A).Amount

                    Call QuitarUserInvItem(VictimIndex, A, .Invent.Object(A).Amount)
        ...
        Call UpdateUserInv(True, VictimIndex, 0)
        
        UserList(UserIndex).flags.ClainObject = 1
...
Retos_SetObj_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mRetos.Retos_SetObj " & "at line " & Erl
...
Sub Retos_ReclameObj(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        For A = 1 To MAX_INVENTORY_SLOTS
            ObjIndex = .ObjectClaim(A).ObjIndex
            
            If ObjIndex > 0 Then
                Obj.ObjIndex = ObjIndex
                Obj.Amount = .ObjectClaim(A).Amount
                
                If MeterItemEnInventario(UserIndex, Obj) Then
                    Call WriteConsoleMsg(UserIndex, "Has reclamado: " & ObjData(ObjIndex).Name & " (x" & Obj.Amount & ")", FontTypeNames.FONTTYPE_INFOGREEN)
                Else
                    Call Logs_User(.Name, eLog.eUser, eReclameObj, .Name & " no recibió el objeto n°" & ObjIndex & "(" & ObjData(ObjIndex).Name & ") x" & Obj.Amount)
                    Call LogRetos("El personaje " & .Name & " con IP: " & .IpAddress & " reclamo un objeto el cual no pudo ser entregado por falta de espacio.")

                End If

            End If
            
            .ObjectClaim(A).ObjIndex = 0
            .ObjectClaim(A).Amount = 0
            .ObjectClaim(A).Equipped = 0
        Next A
...
Retos_ReclameObj_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mRetos.Retos_ReclameObj " & "at line " & Erl
...
Retos_Reset_All_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mRetos.Retos_Reset_All " & "at line " & Erl
