...
RetoFast_UserAbandonate_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mRetoFast.RetoFast_UserAbandonate " & "at line " & Erl
...
Public Sub RetoFast_UserDie(ByVal UserIndex As Integer, _
    ...
    With UserList(UserIndex)
        ...
        For A = LBound(RetoFast(SlotFast).Users) To UBound(RetoFast(SlotFast).Users)
            ...
            If RetoFast(SlotFast).Users(A) > 0 Then
                ...
                If UBound(RetoFast(SlotFast).Users) <= 2 Then
                    Call WriteUpdateEvent(UserList(RetoFast(SlotFast).Users(A)).ID, UserList(RetoFast(SlotFast).Users(A)).Name, eSubType_Modality.Fast1vs1, FightTeam <> UserList(RetoFast(SlotFast).Users(A)).flags.FightTeam)
        ...
        SendData SendTarget.toMap, 1, PrepareMessageConsoleMsg("RetoFast #" & TempIdent & "» " & TempOne & " vs " & TempTwo & ". Gana " & IIf((FightTeam = 2), TempOne, TempTwo), FontTypeNames.FONTTYPE_CONSEJO)
...
Public Sub Rachas_User_Add(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If .flags.RachasTemp Mod 20 = 0 Then
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Rachas» El personaje '" & UCase$(.Name) & "' alcanza las " & .flags.RachasTemp & " rachas. (Record: " & .flags.Rachas & ")", FontTypeNames.FONTTYPE_USERPREMIUM))

        End If
        
        ' # Mensaje SPAM de rachas historicas cada 100
        If .flags.Rachas Mod 100 = 0 Then
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Rachas» '" & UCase$(.Name) & "' alcanza las " & .flags.Rachas & " rachas.", FontTypeNames.FONTTYPE_ANGEL))
...
Public Sub Fast_Reset_User(ByVal Slot As Byte, ByVal SlotUser As Byte)
    ...
    With RetoFast(Slot)
        UserList(.Users(SlotUser)).flags.SlotFast = 0
        UserList(.Users(SlotUser)).flags.SlotFastUser = 0
        UserList(.Users(SlotUser)).flags.FightTeam = 0
        ...
        EventWarpUser .Users(SlotUser), 1, 73 + SlotUser, 73
...
Fast_Reset_User_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mRetoFast.Fast_Reset_User " & "at line " & Erl
...
Fast_Reset_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mRetoFast.Fast_Reset " & "at line " & Erl
...
Fast_Reset_All_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mRetoFast.Fast_Reset_All " & "at line " & Erl
