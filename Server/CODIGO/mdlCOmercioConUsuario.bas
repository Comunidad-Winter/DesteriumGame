...
EnviarOferta_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mdlCOmercioConUsuario.EnviarOferta " & "at line " & Erl & " Cuenta: " & UserList(UserIndex).Account.Email
...
FinComerciarUsu_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mdlCOmercioConUsuario.FinComerciarUsu " & "at line " & Erl
...
Public Sub AceptarComercioUsu(ByVal UserIndex As Integer)
    ...
    For OfferSlot = 1 To MAX_OFFER_SLOTS + 2
        ...
        With UserList(UserIndex)
            ...
            If OfferSlot = GOLD_OFFER_SLOT Then
                ...
                ' Log
                If .ComUsu.GoldAmount > MAX_ORO_LOGUEABLE Then Call Logs_User(.Name, eLog.eUser, eDropGld, .Name & " soltó oro en comercio seguro con " & UserList(OtroUserIndex).Name & ". Cantidad: " & .ComUsu.GoldAmount)
                ' Update Usuario
            ...
            ElseIf OfferSlot = ELDHIR_OFFER_SLOT Then
                ' Quito la cantidad de oro ofrecida
                .Stats.Eldhir = .Stats.Eldhir - .ComUsu.EldhirAmount

                ' Log
                If .ComUsu.EldhirAmount > MAX_ORO_LOGUEABLE Then Call Logs_User(.Name, eLog.eUser, eDropEldhir, .Name & " soltó Eldhir en comercio seguro con " & UserList(OtroUserIndex).Name & ". Cantidad: " & .ComUsu.EldhirAmount)
                ' Update Usuario
                Call WriteUpdateUserStats(UserIndex)
                ' Se la doy al otro
                UserList(OtroUserIndex).Stats.Eldhir = UserList(OtroUserIndex).Stats.Eldhir + .ComUsu.EldhirAmount
                ' Update Otro Usuario
                Call WriteUpdateUserStats(OtroUserIndex)
            ...
            ElseIf .ComUsu.Objeto(OfferSlot) > 0 Then
                ...
                If ObjData(TradingObj.ObjIndex).OBJType = otTransformVIP Then
                ...
                If ObjData(TradingObj.ObjIndex).Log = 1 Then
                    Call Logs_User(.Name, eLog.eUser, eCommerce_Obj, .Name & " le pasó en comercio seguro a " & UserList(OtroUserIndex).Name & " " & TradingObj.Amount & " " & ObjData(TradingObj.ObjIndex).Name)
                ...
                If TradingObj.Amount > MAX_OBJ_LOGUEABLE Then
                    ...
                    If ObjData(TradingObj.ObjIndex).NoLog <> 1 Then
                        Call Logs_User(UserList(OtroUserIndex).Name, eLog.eUser, eCommerce_Obj, UserList(OtroUserIndex).Name & " le pasó en comercio seguro a " & .Name & " " & TradingObj.Amount & " " & ObjData(TradingObj.ObjIndex).Name)
        ...
        With UserList(OtroUserIndex)
            ...
            If OfferSlot = GOLD_OFFER_SLOT Then
                ...
                If .ComUsu.GoldAmount > MAX_ORO_LOGUEABLE Then Call Logs_User(.Name, eLog.eUser, eDropGld, .Name & " soltó oro en comercio seguro con " & UserList(UserIndex).Name & ". Cantidad: " & .ComUsu.GoldAmount)
                ' Update Usuario
                Call WriteUpdateUserStats(OtroUserIndex)
                'y se la doy al otro
                UserList(UserIndex).Stats.Gld = UserList(UserIndex).Stats.Gld + .ComUsu.GoldAmount
                
                If .ComUsu.GoldAmount > MAX_ORO_LOGUEABLE Then Call Logs_User(UserList(UserIndex).Name, eLog.eUser, eDropGld, UserList(UserIndex).Name & " recibió oro en comercio seguro con " & .Name & ". Cantidad: " & .ComUsu.GoldAmount)
                ' Update Otro Usuario
                Call WriteUpdateUserStats(UserIndex)
            ...
            ElseIf OfferSlot = ELDHIR_OFFER_SLOT Then
                ...
                If .ComUsu.EldhirAmount > MAX_ORO_LOGUEABLE Then Call Logs_User(.Name, eLog.eUser, eDropEldhir, .Name & " soltó Eldhir en comercio seguro con " & UserList(UserIndex).Name & ". Cantidad: " & .ComUsu.EldhirAmount)
                ' Update Usuario
                Call WriteUpdateUserStats(OtroUserIndex)
                'y se la doy al otro
                UserList(UserIndex).Stats.Eldhir = UserList(UserIndex).Stats.Eldhir + .ComUsu.EldhirAmount
                
                If .ComUsu.GoldAmount > MAX_ORO_LOGUEABLE Then Call Logs_User(UserList(UserIndex).Name, eLog.eUser, eDropEldhir, UserList(UserIndex).Name & " recibió Eldhir en comercio seguro con " & .Name & ". Cantidad: " & .ComUsu.EldhirAmount)
                ' Update Otro Usuario
                Call WriteUpdateUserStats(UserIndex)
            ...
            ElseIf .ComUsu.Objeto(OfferSlot) > 0 Then
                ...
                If ObjData(TradingObj.ObjIndex).OBJType = otTransformVIP Then
                    If UserList(OtroUserIndex).flags.TransformVIP = 1 Then
                        Call TransformVIP_User(OtroUserIndex, 0)
                ...
                'Es un Objeto que tenemos que loguear? Pablo (ToxicWaste) 07/09/07
                If ObjData(TradingObj.ObjIndex).Log = 1 Then
                    Call Logs_User(.Name, eLog.eUser, eCommerce_Obj, .Name & " le pasó en comercio seguro a " & UserList(UserIndex).Name & " " & TradingObj.Amount & " " & ObjData(TradingObj.ObjIndex).Name)
...
AceptarComercioUsu_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mdlCOmercioConUsuario.AceptarComercioUsu " & "at line " & Erl
    ...
    If PuedeSeguirComerciando(UserIndex) Then
        ...
        With UserList(UserIndex).ComUsu
            ...
            If Not .Confirmo Then
                ...
                ElseIf IsEldhir Then
                    ' Agregamos (o quitamos) mas oro a la oferta
                    .EldhirAmount = .EldhirAmount + Amount
                    
                    ' Imposible que pase, pero por las dudas..
                    If .EldhirAmount < 0 Then .EldhirAmount = 0
...
AgregarOferta_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mdlCOmercioConUsuario.AgregarOferta " & "at line " & Erl
...
PuedeSeguirComerciando_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mdlCOmercioConUsuario.PuedeSeguirComerciando " & "at line " & Erl
...
Private Function HasOfferedItems(ByVal UserIndex As Integer) As Boolean
    ...
    With UserList(UserIndex).ComUsu
        ...
        If UserList(UserIndex).Stats.Eldhir < .EldhirAmount Then Exit Function
...
HasOfferedItems_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.mdlCOmercioConUsuario.HasOfferedItems " & "at line " & Erl
