...
Public Sub Events_Loop_Check()
    ...
    For A = 1 To EventLastDefined

        With EventsCheck(A)

            If .Run = False Then
                Call Events_CheckTime(A)
...
Events_Loop_Check_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_Loop_Check " & "at line " & Erl
...
Private Sub Events_CheckTime(ByVal SlotEvent As Byte)
    ...
    With EventsCheck(SlotEvent).Time
        
        'EXAMPLE:  1° de cada mes
        If .InitDay > 0 Then
            If Day(Time) <> .InitDay Then Exit Sub

        End If
        
        ' Días: SABADO
        If .InitWeek > 0 Then
            If Weekday(Time) <> .InitWeek Then Exit Sub

        End If
        
        ' Horario específico: 18hs
        If .InitHour > 0 Then
            If Hour(Time) <> .InitHour Then Exit Sub
        ...
        If .InitMinute > 0 And .Fail = 0 Then
            If Minute(Time) <> .InitMinute Then Exit Sub

        End If
        
        Dim Slot As Byte

        Slot = NewEvent(EventsCheck(SlotEvent))
        
        If Slot = 0 Then
            .Fail = .Fail + 1
            
            If .Fail = 10 Then .Fail = 0
        Else
            EventsCheck(SlotEvent).Run = True
...
Events_CheckTime_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_CheckTime " & "at line " & Erl
...
Public Sub Events_Read()
    ...
    Call Read.Initialize(App.Path & "\dat\events\events_list.ini")
    
    EventLastDefined = val(Read.GetValue("INIT", "LAST"))
    
    ReDim EventsCheck(0 To EventLastDefined) As tEvents
    ...
    For A = 1 To EventLastDefined
        ...
        With EventsCheck(A)
            ...
            .Name = Read.GetValue(A, "NAME")
            .Quotas = Read.GetValue(A, "QUOTAS")
            .TeamCant = Read.GetValue(A, "TEAMCANT")
            .Modality = val(Read.GetValue(A, "MODALITY"))

            .PrizeGld = val(Read.GetValue(A, "REWARDGLD"))
            .LimitRound = val(Read.GetValue(A, "ROUNDS"))
            .LimitRoundFinal = val(Read.GetValue(A, "ROUNDSFINAL"))
            
            .config(eConfigEvent.eParty) = val(Read.GetValue(A, "CONPARTY"))
            .config(eConfigEvent.eBronce) = val(Read.GetValue(A, "ITEMBRONCE"))
            .config(eConfigEvent.ePlata) = val(Read.GetValue(A, "ITEMPLATA"))
            .config(eConfigEvent.eOro) = val(Read.GetValue(A, "ITEMORO"))
            .config(eConfigEvent.ePremium) = val(Read.GetValue(A, "ITEMPREMIUM"))
            
            .config(eConfigEvent.eUseParalizar) = val(Read.GetValue(A, "UseParalizar"))
            .config(eConfigEvent.eUseApocalipsis) = val(Read.GetValue(A, "UseApocalipsis"))
            .config(eConfigEvent.eUseDescarga) = val(Read.GetValue(A, "UseDescarga"))
            .config(eConfigEvent.eUseTormenta) = val(Read.GetValue(A, "UseTormenta"))
            
            Temp = Read.GetValue(A, "LVL")
            .LvlMin = val(ReadField(1, Temp, 45))
            .LvlMax = val(ReadField(2, Temp, 45))
            
            Temp = Read.GetValue(A, "REWARDOBJ")
            
            .PrizeObj.ObjIndex = val(ReadField(1, Temp, 45))
            .PrizeObj.Amount = val(ReadField(2, Temp, 45))
            
            ' Día Especifico (número)
            ' Día Específico (lunes, martes, miercoles, sabado)
            ' Horario Específico (18:00hs)
            .Time.InitDay = val(Read.GetValue(A, "INITDAY"))
            .Time.InitWeek = val(Read.GetValue(A, "INITWEEK"))
            .Time.InitHour = val(Read.GetValue(A, "INITHOUR"))
            .Time.InitMinute = val(Read.GetValue(A, "INITMINUTE"))
            
            Temp = Read.GetValue(A, "Class")

            Dim TempClass   As eClass
            
            Dim ArrayTemp() As String

            ArrayTemp = Split(Read.GetValue(A, "Class"), "-")
            
            For B = LBound(ArrayTemp) To UBound(ArrayTemp)
                .AllowedClasses(val(ArrayTemp(B))) = 1
            Next B
            ...
            For B = 1 To MAX_REWARD_OBJ
                Temp = Read.GetValue(A & "-REWARD", B)
                
                .RewardObj(B).ObjIndex = val(ReadField(1, Temp, 45))
                .RewardObj(B).Amount = val(ReadField(2, Temp, 45))
                
                If .RewardObj(B).ObjIndex > 0 Then .LastReward = .LastReward + 1
...
Events_Read_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_Read " & "at line " & Erl
...
Public Sub Events_Data_Predetermined()
    ...
    Call Manager.Initialize(App.Path & "\dat\events\events_list.ini")
    ...
    For A = 1 To EventLastDefined
        ...
        With Events(A)
            ...
            For B = 1 To MAX_REWARD_OBJ
                Call Manager.ChangeValue(A & "-REWARD", B, CStr(.RewardObj(B).ObjIndex & "-" & CStr(.RewardObj(B).Amount)))
            Next B
    ...
    Call Manager.DumpFile(App.Path & "\dat\events\events_list.ini")
...
Events_Data_Predetermined_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_Data_Predetermined " & "at line " & Erl
...
FreeSlotEvent_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.FreeSlotEvent " & "at line " & Erl
...
Private Function Event_ModalityRepeat(ByVal Modality As eModalityEvent) As Boolean
    ...
    For LoopC = 1 To MAX_EVENT_SIMULTANEO
        ...
        If Events(LoopC).Modality = Modality Then
            Event_ModalityRepeat = True
...
Event_ModalityRepeat_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Event_ModalityRepeat " & "at line " & Erl
...
Private Function Event_NameRepeat(ByVal Name As String) As Boolean
    ...
    For LoopC = 1 To MAX_EVENT_SIMULTANEO
        ...
        If StrComp(UCase$(Events(LoopC).Name), UCase$(Name)) = 0 Then
            Event_NameRepeat = True
...
Event_NameRepeat_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Event_NameRepeat " & "at line " & Erl
...
Private Function FreeSlotUser(ByVal SlotEvent As Byte) As Byte
    ...
    With Events(SlotEvent)
        ...
        For LoopC = 1 To MAX_USERS_EVENT
            ...
            If .Users(LoopC).Name = vbNullString Then
                FreeSlotUser = LoopC
...
FreeSlotUser_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.FreeSlotUser " & "at line " & Erl
...
Private Function FreeSlotArena(ByVal SlotEvent As Byte) As Byte
    ...
    With Events(SlotEvent)
        ...
        Do While Not Searched
            Temp = RandomNumber(.ArenasMin, .ArenasMax)
 
            If MapEvent.Fight(Temp).Run = False And .ArenasOcupadas < .ArenasLimit Then
                .ArenasOcupadas = .ArenasOcupadas + 1
                Searched = True
                Exit Do
            Else
...
strUsersEvent_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.strUsersEvent " & "at line " & Erl
...
CheckAllowedClasses_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.CheckAllowedClasses " & "at line " & Erl
...
Private Function Events_CheckAllowed_Faction(ByRef AllowedFaction() As Byte) As String
    ...
    For LoopC = 1 To 4
        ...
        If AllowedFaction(LoopC) = 1 Then
...
Events_CheckAllowed_Faction_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_CheckAllowed_Faction " & "at line " & Erl
...
Events_SearchSlotEvent_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_SearchSlotEvent " & "at line " & Erl
...
strModality_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.strModality " & "at line " & Erl
...
Private Function strDescEvent(ByVal SlotEvent As Byte, _
    ...
    Select Case Modality
        ...
        Case eModalityEvent.Enfrentamientos
            ...
            If Events(SlotEvent).TeamCant = 1 Then
                strDescEvent = "» Los usuarios combatirán en duelos 1vs1"
            Else
                strDescEvent = "» Los usuarios combatirán en duelos " & Events(SlotEvent).TeamCant & "vs" & Events(SlotEvent).TeamCant & " donde se escogerán las parejas al azar."
...
strDescEvent_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.strDescEvent " & "at line " & Erl
...
Private Sub InitEvent(ByVal SlotEvent As Byte)
    ...
    Select Case Events(SlotEvent).Modality
        ...
        Case eModalityEvent.JuegosDelHambre
            Call Events_JDH_Init(SlotEvent)
...
error:
    LogEventos "[" & Err.number & "] " & Err.description & ") PROCEDIMIENTO : InitEvent() EN EL EVENTO " & Events(SlotEvent).Modality & "."
    '<EhFooter>
    Exit Sub

InitEvent_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.InitEvent " & "at line " & Erl
...
error:
    LogEventos "[" & Err.number & "] " & Err.description & ") PROCEDIMIENTO : CanAttackUserEvent()"
    '<EhFooter>
    Exit Function

CanAttackUserEvent_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.CanAttackUserEvent " & "at line " & Erl
...
Private Function PrizeUser_All(ByVal SlotEvent As Byte, _
    ...
    With Events(SlotEvent)
        ...
        For A = LBound(.Users) To UBound(.Users)
            ...
            If .Users(A).ID > 0 Then
                If Team = 0 Or Team = .Users(A).Team Then
                          
                    Call PrizeUser(.Users(A).ID, Bonus)
                    
                    Text = Text & UserList(.Users(A).ID).Name & ", "
        ...
        If Len(Text) > 2 Then Text = Left$(Text, Len(Text) - 2)
...
PrizeUser_All_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.PrizeUser_All " & "at line " & Erl
...
Private Sub ChangeBodyEvent(ByVal UserIndex As Integer, ByVal ChangeHead As Boolean)
...
Public Function Events_GenerateSpamShort(ByVal Slot As Byte) As String
    ...
    With Events(Slot)
        ...
        Events_GenerateSpamShort = "Evento '" & UCase$(.Name) & IIf(.config(eConfigEvent.eParty) = 1, " (Party)", "") & ". " & strTemp & "Ganá hasta " & IIf(.PrizePoints > 0, .PrizePoints & " Puntos de Torneo", "0 Puntos") & ". Únete con '/TORNEO " & UCase$(.Name) & "'." & vbCrLf & "El evento comienza en " & SecondsToHMS(.TimeCancel)
...
Private Function Events_CheckNew(ByRef Data As tEvents) As Boolean
    ...
    Events_CheckNew = False

    If Data.PrizeObj.ObjIndex > NumObjDatas Then Exit Function
    If Data.PrizeObj.Amount < 0 Or Data.PrizeObj.Amount > 1000 Then Exit Function
    Events_CheckNew = True
    
    '<EhFooter>
    Exit Function

Events_CheckNew_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_CheckNew " & "at line " & Erl
...
Public Function Events_Group_CheckUsers(ByVal GroupIndex As Integer, _
                                        ByVal SlotEvent As Byte, _
                                        ByRef ErrorMsg As String) As Boolean
...
Public Sub Event_SetNewUser(ByVal UserIndex As Integer, ByVal SlotEvent As Byte)
    ...
    Message = "Partida " & Events(SlotEvent).Name & "» Se anota " & UserList(UserIndex).Name & " [" & ListaClases(UserList(UserIndex).Clase) & " " & ListaRazas(UserList(UserIndex).Raza) & "] Inscriptos TOTALES: " & Events(SlotEvent).Inscribed
...
Event_SetNewUser_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Event_SetNewUser " & "at line " & Erl
        
    '</EhFooter>
End Sub

' # Busca un usuario en el evento al que quiere participar.
Public Function Event_CheckExistUser(ByVal UserIndex As Integer, _
                                     ByVal SlotEvent As Byte) As Integer
...
Event_SetNewUser_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Event_CheckExistUser " & "at line " & Erl
...
Public Sub ParticipeEvent_User(ByVal UserIndex As Integer, ByVal SlotEvent As Byte)
    ...
    With UserList(UserIndex)
        ...
        Pos.Map = MapEvent.SalaEspera.Map
        Pos.X = MapEvent.SalaEspera.X
        Pos.Y = MapEvent.SalaEspera.Y
...
Public Function Event_CheckInscriptions_User(ByVal UserIndex As Integer, _
                                             ByVal SlotEvent As Byte, _
                                             ByRef ErrorMsg As String) As Boolean
...
Public Sub Event_Initial(ByVal SlotEvent As Byte, ByVal Inscribed As Byte)
    ...
    With Events(SlotEvent)
        ...
        If .PrizePoints > 0 Then .PrizePoints = Events_SetReward_Points(Events(SlotEvent), .Quotas)
...
Event_Initial_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Event_Initial " & "at line " & Erl
...
Event_ClassOld_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Event_ClassOld " & "at line " & Erl
...
Private Sub Events_FinishTime(ByVal SlotEvent As Byte)
    ...
    With Events(SlotEvent)
        ...
        Select Case .Modality
            ...
            Case eModalityEvent.Unstoppable, eModalityEvent.Busqueda
...
CanAttackReyCastle_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.CanAttackReyCastle " & "at line " & Erl
...
Public Sub DataRusa_SummonUser(ByVal SlotEvent As Byte, Optional ByVal Blocked As Boolean)
    ...
    With Events(SlotEvent)
        ...
        For A = LBound(.Users()) To UBound(.Users())
            ...
            If .Users(A).ID > 0 Then
                If Blocked Then
                    Call WriteUserInEvent(.Users(A).ID)
                ...
                Call WarpUserChar(.Users(A).ID, MapEvent.DagaRusa.Map, MapEvent.DagaRusa.X + Num, MapEvent.DagaRusa.Y, False)
...
DataRusa_SummonUser_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.DataRusa_SummonUser " & "at line " & Erl
...
DagaRusa_NextUser_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.DagaRusa_NextUser " & "at line " & Erl
...
Private Sub Events_Finish(ByVal SlotEvent As Byte, ByVal TeamUser As Byte)
    ...
    With Events(SlotEvent)
        ...
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(TextWinner, FontTypeNames.FONTTYPE_EVENT))
        
        Call PrizeUser_All(SlotEvent, TeamUser)
        Call CloseEvent(SlotEvent)
...
Events_Finish_Err:
    LogError Err.description & vbCrLf & "in Events_Finish " & "at line " & Erl

    '</EhFooter>
End Sub

' # Agrega un Round ganador al team seleccionado
Private Function Events_AddRound(ByVal SlotEvent As Byte, ByVal Team As Byte) As Integer
    ...
    With Events(SlotEvent)
        ...
        For A = LBound(.Users) To UBound(.Users)
            ...
            If .Users(A).Team = Team Then
                .Users(A).RoundsWin = .Users(A).RoundsWin + 1
                .Users(A).RoundsWinFinal = .Users(A).RoundsWinFinal + 1
                Temp = .Users(A).RoundsWin

            End If

        Next A
        
        Events_AddRound = Temp
...
Events_AddRound_Err:
    LogError Err.description & vbCrLf & "in Events_AddRound " & "at line " & Erl
...
Private Function Event_SearchOponent(ByVal SlotEvent As Byte, ByVal Team As Byte) As Byte
    ...
    With Events(SlotEvent)
        ...
        For A = LBound(.Users) To UBound(.Users)
            ...
            If .Modality = Enfrentamientos Then
                If .Users(A).ID > 0 And .Users(A).Oponent = Team Then
                    Event_SearchOponent = .Users(A).Team
                    Exit Function
            ...
            Else
                ...
                If .Users(A).ID > 0 And .Users(A).Team <> Team Then
                    Event_SearchOponent = A
                    Exit Function
...
Event_SearchOponent_Err:
    LogError Err.description & vbCrLf & "in Event_SearchOponent " & "at line " & Erl
...
Public Function Events_Generate_String_Wins(ByVal NameEvent As String, _
                                            ByVal Points As Integer, _
                                            ByVal QuotasMax As Integer, _
                                            ByRef Users() As tUserEvent, _
                                            ByVal CountWin As Integer, _
                                            ByVal TeamWin As Byte)
...
Public Sub Events_CheckInscribed(ByVal UserIndex As Integer, _
    ...
    Oponent = Event_SearchOponent(SlotEvent, TeamUser)
    
    With Events(SlotEvent)

        Select Case .Modality

            Case eModalityEvent.DagaRusa, eModalityEvent.CastleMode, eModalityEvent.Busqueda, eModalityEvent.Unstoppable, eModalityEvent.JuegosDelHambre, eModalityEvent.DeathMatch
                
                If Oponent = 0 Then
                    Call CloseEvent(SlotEvent)
                Else

                    If .Inscribed <= TeamCant(SlotEvent, Oponent) Then
                        Call Events_Finish(SlotEvent, Oponent)
            ...
            Case eModalityEvent.Enfrentamientos
                ...
                If Not Fight_CheckContinue(UserIndex, SlotEvent, TeamUser) Then
                    UserCheck = Events_AddRound(SlotEvent, Oponent)
                            
                    If UserCheck >= (.LimitRound / 2) + 0.5 Then
...
CheckGanador:
                        ...
                        If MapFight > 0 Then
                            ...
                            Call Fight_TeamWin(SlotEvent, Oponent, strWin)
                            Text = Events(SlotEvent).Name & "» Gana " & strWin & "."
                            Call Team_UserDie(SlotEvent, TeamUser, Events(SlotEvent).Users)
                        ...
                        If .Inscribed <= TeamCant(SlotEvent, Oponent) Then
                            ...
                            Call CloseEvent(SlotEvent, Text)
                        Else
                            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(Text, FontTypeNames.FONTTYPE_EVENT, eMessageType.cEvents_Curso))
                            Call NewRound(SlotEvent)
                            Call Fight_Combate(SlotEvent)

                        End If

                    Else

                        If Not Fight_CheckContinue(UserIndex, SlotEvent, TeamUser, 1) Then
                            GoTo CheckGanador:
                        Else
                            Fight_WarpTeam SlotEvent, MapFight, TeamUser, True, vbNullString, Oponent, False
                            Fight_WarpTeam SlotEvent, MapFight, Oponent, False, vbNullString, TeamUser, False
...
Events_CheckInscribed_Err:
    LogError Err.description & vbCrLf & "in Events_CheckInscribed " & "at line " & Erl
    Call CloseEvent(SlotEvent)
...
Public Sub Events_UserDie(ByVal UserIndex As Integer, _
    ...
    SlotEvent = UserList(UserIndex).flags.SlotEvent
    SlotEventUser = UserList(UserIndex).flags.SlotUserEvent
    TeamUser = Events(SlotEvent).Users(SlotEventUser).Team
    TeamOponent = Events(SlotEvent).Users(SlotEventUser).Oponent
    MapFight = Events(SlotEvent).Users(SlotEventUser).MapFight
    ...
    Select Case Events(SlotEvent).Modality
        ...
        Case eModalityEvent.DeathMatch, eModalityEvent.GranBestia, eModalityEvent.JuegosDelHambre, eModalityEvent.DagaRusa
            ...
            Call Events_CheckInscribed(UserIndex, SlotEvent, 0, TeamUser, 0)
        
        Case eModalityEvent.Unstoppable

            If AttackerIndex = 0 Then Exit Sub
            If General.AntiFrags_CheckUser(AttackerIndex, UserIndex, 20) Then
                Events(SlotEvent).Users(UserList(AttackerIndex).flags.SlotUserEvent).Value = Events(SlotEvent).Users(UserList(AttackerIndex).flags.SlotUserEvent).Value + 1

            End If
            
            Call Events_UserRevive(UserIndex)
            
        Case eModalityEvent.CastleMode
            Call Events_UserRevive(UserIndex)
        
        Case eModalityEvent.Enfrentamientos
            Call Events_CheckInscribed(UserIndex, SlotEvent, SlotEventUser, TeamUser, MapFight)
...
Events_UserDie_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_UserDie " & "at line " & Erl
...
Public Sub Events_ChangeApparience(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        Call ChangeBodyEvent(UserIndex, True)
        .ShowName = False
        
        If .flags.SlotEvent Then
            If Events(.flags.SlotEvent).config(eConfigEvent.eMezclarApariencias) = 1 Then
                .Counters.TimeApparience = 1
...
Events_ChangeApparience_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_ChangeApparience " & "at line " & Erl
...
Public Sub Events_ChangePosition(ByVal UserIndex As Integer, ByVal SlotEvent As Byte)
    ...
    If Events(SlotEvent).Inscribed = TeamCant(SlotEvent, UserIndex) Then Exit Sub
    
    Select Case Events(SlotEvent).Modality

        Case eModalityEvent.DeathMatch
            Pos.Map = MapEvent.DeathMatch.Map
            Pos.X = RandomNumber(MapEvent.DeathMatch.X - 10, MapEvent.DeathMatch.X + 10)
            Pos.Y = RandomNumber(MapEvent.DeathMatch.Y - 10, MapEvent.DeathMatch.Y + 10)
            
            If Events(SlotEvent).config(eConfigEvent.eTeletransportacion) = 1 Then
                UserList(UserIndex).Counters.TimeTelep = 15

            End If

    End Select
    
    If Pos.Map Then Call EventWarpUser(UserIndex, Pos.Map, Pos.X, Pos.Y)
    '<EhFooter>
    Exit Sub

Events_ChangePosition_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_ChangePosition " & "at line " & Erl
    ...
    With Events(SlotEvent)
        ...
        For LoopC = LBound(.Users()) To UBound(.Users())
            ...
            If .Users(LoopC).ID > 0 Then
                ...
                ' Hay luchando
                If .Users(LoopC).MapFight > 0 Then
...
NewRound_Err:
    LogError Err.description & vbCrLf & "in NewRound " & "at line " & Erl
...
Private Sub Team_UserDie(ByVal SlotEvent As Byte, _
    ...
    For LoopC = LBound(Users()) To UBound(Users())
        ...
        If Not Events(SlotEvent).Run Then Exit Sub ' Si ya cerró el evento significa que no hace falta seguir recorriendo.
        
        If Users(LoopC).ID > 0 Then
            If Users(LoopC).Team = TeamSlot Then
                UserIndex = Users(LoopC).ID
...
Team_UserDie_Err:
    LogError Err.description & vbCrLf & "in Team_UserDie " & "at line " & Erl
...
Public Function Fight_CheckContinue(ByVal UserIndex As Integer, _
    ...
    With Events(SlotEvent)
        ...
        Fight_CheckContinue = False
              
        For LoopC = LBound(.Users()) To UBound(.Users())
            
            ' User válido
            If .Users(LoopC).ID > 0 And (.Users(LoopC).ID <> UserIndex Or Muerto = 1) Then
                If .Users(LoopC).Team = TeamSlot Then
                    If (UserList(.Users(LoopC).ID).flags.Muerto = 0) Or Muerto = 1 Then
                        Fight_CheckContinue = True
...
Fight_CheckContinue_Err:
    LogError Err.description & vbCrLf & "in Fight_CheckContinue " & "at line " & Erl
        ...
        For LoopC = LBound(.Users()) To UBound(.Users())
            ...
            If .Users(LoopC).ID > 0 Then
                ...
                With .Users(LoopC)
                    ...
                    If .Team = TeamSlotOponent Then
                        Events_StatsFull .ID, False
                        ...
                        EventWarpUser .ID, MapEvent.SalaEspera.Map, MapEvent.SalaEspera.X, MapEvent.SalaEspera.Y
                        WriteConsoleMsg .ID, "Felicitaciones. Has ganado el duelo", FontTypeNames.FONTTYPE_INFO
                              
                        UserList(.ID).flags.FightTeam = 0
                        RefreshCharStatus (.ID)
...
Fight_TeamWin_Err:
    LogError Err.description & vbCrLf & "in Fight_TeamWin " & "at line " & Erl
...
Private Function Events_StrReward(ByVal SlotEvent As Byte) As String
    ...
    With Events(SlotEvent)
        ...
        If .PrizeGld = 0 And .PrizeEldhir = 0 And .PrizePoints = 0 And .PrizeObj.ObjIndex = 0 Then
            Temp = "Premio entregado: NINGUNO"
            Exit Function
        Else
            Temp = "Premio entregado: "
            
            If .PrizeGld > 0 Then
                Temp = Temp & .PrizeGld & " Monedas de Oro. "

            End If
                    
            If .PrizeEldhir > 0 Then
                Temp = Temp & .PrizeEldhir & " DSP. "
            ...
            If .PrizeObj.ObjIndex > 0 Then
                Temp = Temp & ObjData(.PrizeObj.ObjIndex).Name & " (x" & .PrizeObj.Amount & ")"

            End If

        End If
        
        Events_StrReward = Temp
...
Events_StrReward_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_StrReward " & "at line " & Erl
...
Public Function NewEvent_Configuration(ByVal Name As String, _
    ...
    NewEvent_Configuration = Temp
    '<EhFooter>
    Exit Function

NewEvent_Configuration_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.NewEvent_Configuration " & "at line " & Erl
...
Event_Text_Users_VS_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Event_Text_Users_VS " & "at line " & Erl
...
Public Sub Events_Teleports_Init(ByVal SlotEvent As Byte)
    ...
    With Events(SlotEvent)
        ...
        For A = LBound(.Users()) To UBound(.Users())
            ...
            If .Users(A).ID > 0 Then
                EventWarpUser .Users(A).ID, MapEvent.Teleport(MapIndex).Map, MapEvent.Teleport(MapIndex).XWarp, MapEvent.Teleport(MapIndex).YWarp
...
Events_Teleports_Init_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_Teleports_Init " & "at line " & Erl
...
Public Sub Events_Teleports_Finish(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        Call WriteConsoleMsg(UserIndex, "¡¡Felicitaciones!! Has ganado el Evento de Teleports.", FontTypeNames.FONTTYPE_CONSEJOCAOS)
        Call PrizeUser(UserIndex, 0)
        Call CloseEvent(.flags.SlotEvent, strModality(.flags.SlotEvent, eModalityEvent.Teleports) & "» El personaje " & .Name & " ha encontrado el Teleport Final. ¡¡Felicitaciones!!. " & Events_StrReward(.flags.SlotEvent))
...
Events_Teleports_Finish_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_Teleports_Finish " & "at line " & Erl
...
Public Sub Events_GranBestia_MuereNpc(ByVal UserIndex As Integer)
    ...
    With Events(SlotEvent)
        Call PrizeUser(UserIndex, 0)
        strTemp(0) = strModality(SlotEvent, eModalityEvent.GranBestia) & "» El personaje " & UserList(UserIndex).Name & " ha acabado con la Gran Bestia. " & Events_StrReward(SlotEvent)
...
Events_GranBestia_MuereNpc_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_GranBestia_MuereNpc " & "at line " & Erl
...
Events_GranBestia_MuereUser_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_GranBestia_MuereUser " & "at line " & Erl
...
SearchLastUserEvent_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.SearchLastUserEvent " & "at line " & Erl
...
Private Function Event_GenerateTablaPos(ByVal SlotEvent As Byte, _
    ...
    With Events(SlotEvent)
        ...
        For LoopC = LBound(.Users()) To UBound(.Users())
            ...
            If CopyUsers(LoopC).ID > 0 Then
                Event_GenerateTablaPos = Event_GenerateTablaPos & LoopC & "° »» " & UserList(CopyUsers(LoopC).ID).Name & " (" & CopyUsers(LoopC).Value & ")" & vbCrLf
...
Event_GenerateTablaPos_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Event_GenerateTablaPos " & "at line " & Erl
...
Event_RandomUsers_Array_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Event_RandomUsers_Array " & "at line " & Erl
...
Eventos_Reset_All_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Eventos_Reset_All " & "at line " & Erl
...
AssignTeam_Err:
    LogError Err.description & vbCrLf & "in AssignTeam " & "at line " & Erl
    Call CloseEvent(SlotEvent)

End Sub

Private Function GroupProcessed(ByVal GroupIndex As Integer, _
                                ByVal ProcessedGroups As Collection) As Boolean
...
Private Sub AssignGroupToTeam(ByVal GroupIndex As Integer, _
                              ByVal Team As Byte, _
                              ByRef EventData As tEvents)
...
Public Sub Events_CheckUsers(ByVal SlotEvent As Byte, _
                             ByRef Users() As Integer, _
                             ByVal Inscriptos As Integer)
...
Public Sub Events_UpdateUsers(ByVal SlotEvent As Byte, _
                              ByVal Quotas As Integer, _
                              ByRef tUser() As Integer)
    ...
    With Events(SlotEvent)
        ...
        For A = LBound(tUser()) To UBound(tUser())
...
Public Sub Events_GenerateTableWeb()
    ...
    For A = 1 To MAX_EVENT_SIMULTANEO
        ...
        With Events(A)
            ...
            If .PrizeObj.ObjIndex > 0 Then
                TempObj = ObjData(.PrizeObj.ObjIndex).Name & " (x" & .PrizeObj.Amount & ")"
            Else
                TempObj = "(Ninguno)"

            End If
            
            Text = strModality(A, .Modality) & "-" & .Inscribed & "/" & .Quotas & "-" & .LvlMin & "/" & .LvlMax & "-" & .InscriptionGld & "-" & .InscriptionEldhir & "-" & .PrizeGld & "-" & .PrizeEldhir & "-" & TempObj & vbCrLf
...
Events_GenerateTableWeb_Err:
    LogError Err.description & vbCrLf & "in ServidorArgentum.EventosDS.Events_GenerateTableWeb " & "at line " & Erl
        
    '</EhFooter>
End Sub

' # Agrega daño causado guardado
Public Function Events_Add_Damage(ByVal SlotEvent As Byte, _
                                  ByVal SlotEventUser As Byte, _
                                  ByVal Value As Long)
